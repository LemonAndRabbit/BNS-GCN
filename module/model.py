...
class GraphSAGE(GNNBase):
    ...
    def __init__(self, layer_size, activation, use_pp, dropout=0.5, norm='layer', train_size=None, n_linear=0, dropon='agg'):
        super(GraphSAGE, self).__init__(layer_size, activation, use_pp, dropout, norm, n_linear)
        for i in range(self.n_layers):
            if i < self.n_layers - self.n_linear:
                self.layers.append(GraphSAGELayer(layer_size[i], layer_size[i + 1], use_pp=use_pp, dropout=dropout if dropon == 'agg' else 0.))
            else:
    ...
    def forward(self, g, feat, in_deg=None):
        h = feat
        for i in range(self.n_layers):
            if self.dropon == 'sample' or i >= self.n_layers - self.n_linear:
                h = self.dropout(h)
            if i < self.n_layers - self.n_linear:
